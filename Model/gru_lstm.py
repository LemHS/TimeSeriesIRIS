...
def SingleShotGRULSTM(train_x, val_x, train_y, val_y):
    """
    ...
    gru = GRU(128, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.01))(input_ts)
    # Adding Dropout Layer
    dropout_1 = Dropout(0.5)(gru)
...
def Seq2SeqGRULSTM(train_x, val_x, train_y, val_y):
    """
    ...
    seq2seq_model.add(GRU(100, activation='relu', return_sequences=True, input_shape=(n_steps_in, n_features)))
    seq2seq_model.add(GRU(100, activation='relu'))
...
def AutoregressiveGRULSTM(train_x, val_x, train_y, val_y):
    """
    Create an Autoregressive LSTM Model with a Dropout layer and train it using the training & validation data.
    """
    ...
    input_ts = Input(shape=(train_x.shape[1], train_x.shape[2]))
    gru = GRU(128, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.01))(input_ts)
    # gru, state = GRU(128, return_state=True, kernel_regularizer=tf.keras.regularizers.l2(0.01))(input_ts)
    # Adding Dropout Layer
    dropout_1 = Dropout(0.5)(gru)

    x, state = GRU(128, return_state=True)(dropout_1)

    lsmt_cell = LSTMCell(128)
    dense_1 = Dense(32)
    dense_2 = Dense(1)
    ...
    for n in range(1, 3):
        x = prediction
        if n == 1:
          x, state = lsmt_cell(x, states=(state, state))
        else:
          x, state = lsmt_cell(x, states=state)
        x = dense_1(x)
        prediction = dense_2(x)
        predictions.append(prediction)
...
class BuildGRULSTM(keras_tuner.HyperModel):
    ...
    def build(self, hp):
        if self.type == 'SingleShot':
            return self.HypertuneSingleShotGRULSTM(hp)
        elif self.type == 'Seq2Seq':
            return self.HypertuneSeq2SeqGRULSTM(hp)
        elif self.type == 'Autoregressive':
            return self.HypertuneAutoregressiveGRULSTM(hp)
    ...
    def HypertuneSingleShotGRULSTM(self, hp):
        """
        Build a tunable Single Shot GRU-LSTM Model.
        """
        # Hyperparameters
        ...
        # Model Definition
        ss_gru_lstm = Model(inputs=input_ts, outputs=prediction)
    ...
    def HypertuneSeq2SeqGRULSTM(self, hp):
        """
        Build a tunable Seq2Seq GRU-LSTM Model.
        """
        ...
        # Model definition
        seq2seq_model = Sequential()
        seq2seq_model.add(GRU(gru_units, activation='relu', return_sequences=True, input_shape=(n_steps_in, n_features)))
        seq2seq_model.add(GRU(gru_units, activation='relu'))
        seq2seq_model.add(RepeatVector(n_steps_out))
    ...
    def HypertuneAutoregressiveGRULSTM(self, hp):
        """
        Build a tunable Autoregressive GRU-LSTM Model.
        """
        predictions = []
        ...
        x, state = GRU(gru_lstm_units, return_state=True)(dropout_1)

        # LSTM Cell and Dense Layers
        lstm_cell = LSTMCell(gru_lstm_units)
        ...
        for n in range(1, 3):
            x = prediction
            x, state = lstm_cell(x, states=(state, state)) if n == 1 else lstm_cell(x, states=state)
            x = dense_1(x)
            prediction = dense_2(x)
            predictions.append(prediction)
...
# def BuildGRULSTM(train_x, val_x, train_y, val_y, type):
#     def HypertuneSingleShotGRULSTM(hp):
#         """
#         Build a tunable Single Shot GRU-LSTM Model.
#         """
#         # Hyperparameters
...
#     def HypertuneSeq2SeqGRULSTM(hp):
#         """
#         Build a tunable Seq2Seq GRU-LSTM Model.
#         """
#         # Define hyperparameters
#         gru_units = hp.Int('gru_units', min_value=50, max_value=200, step=50)
...
#         # Model definition
#         seq2seq_model = Sequential()
#         seq2seq_model.add(GRU(gru_units, activation='relu', input_shape=(n_steps_in, n_features)))
#         seq2seq_model.add(RepeatVector(n_steps_out))
#         seq2seq_model.add(LSTM(lstm_units, activation='relu', return_sequences=True))
#         seq2seq_model.add(TimeDistributed(Dense(1)))
...
#     def HypertuneAutoregressiveGRULSTM(hp):
#         """
#         Build a tunable Autoregressive GRU-LSTM Model.
#         """
#         predictions = []
...
#     if type == "SingleShot":
#         return HypertuneSingleShotGRULSTM
#     elif type == "Seq2Seq":
#         return HypertuneSeq2SeqGRULSTM
#     else:
#         return HypertuneAutoregressiveGRULSTM